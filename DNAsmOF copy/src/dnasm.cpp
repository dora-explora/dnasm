...
void Ribosome::step(char* incodons, string* indecodons) {
    ribcurrent = indecodons[ribcursor];
    if (commenting) {
        ...
        if (cooldown == 1) {
            if (indecodons[ribcursor - 4] == "RunPro") {
                runproteins();
            } else if (indecodons[ribcursor - 4] == "RibJmp") {
                ribosomejump();
            }
        }
        cooldown--;
        ...
        writing = true;
        workingmarker = incodons[ribcursor+1];
        cooldown = 1;
    } else if (attached && !writing && ribcurrent == "Ready ") {
        readiedmarkers.push_back(incodons[ribcursor]);
        ...
        workingcodons.push_back(incodons[ribcursor]);
        workingdecodons.push_back(indecodons[ribcursor]);
