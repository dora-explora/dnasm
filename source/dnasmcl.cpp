...
void display_enzymes(vector<char> markers, map<char, Enzyme> enzymes) {
    if (!markers.empty()) {
         for (int i = 0; i < (markers.size()); i++) {
             cout << "\nEnzyme #" << i + 1 << " with marker 0b" << bitset<6>(markers[i]) << endl;
...
int main(int argc, char* argv[]) {
    ...
    if (argc == 1) {
        ...
        throw std::invalid_argument("Too many arguments. Only type one argument for the files name.");
    }
    setup(filename);
    bool finished = false;
    string nothing; // dora's home for wayward strings
    Ribosome ribosome;

    while (ticks < 2147483647 && ribosome.ribcursor < (length - 1)) {
        for (char currentmarker : ribosome.runningmarkers) {
            ...
            if (currentenzyme.instrptr >= currentenzyme.decodons.size() || currentenzyme.cursor >= length) { 
                ribosome.runningmarkers.erase(remove(ribosome.runningmarkers.begin(), ribosome.runningmarkers.end(), currentmarker), ribosome.runningmarkers.end());
            } else {
    ...
    delete[] codons;
    delete[] decodons;
    cout << "The program has finished. Press any key then enter to exit.\n";
    cin >> nothing;
    return 0;
}